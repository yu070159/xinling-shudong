...
- **部署同步规则**：只改源项目文件，不要主动同步到 `soul-tree-deploy/`。等用户明确说"同步到部署文件夹"时才执行同步操作。
...
## 2026-05-26 书洞页面全面优化（七次提交）

### 解决的问题
- 书洞 13 本书豆瓣链接全部替换为鸠摩搜书/微信读书/ManyBooks/Internet Archive 等免费平台，每本书使用最合适的平台
- Supabase `resources` 表旧数据覆盖了 `books-data.js` 静态数据，导致豆瓣链接在页面上残留——改为 libraryData 为基础、Supabase 仅补充用户提交的独有资源
- 书籍卡片 `<a href>` 跳转外链频繁 404——改为 `<div>` + 点击弹出站内详情弹窗，用户确认后才打开外链
- 搜索框视觉不醒目、缺少模式切换——升级为广场同款三模式（综合/书名/作者）+ ×清除按钮

### 做的修改
- **新建** `books-data.js` — 171 本心理学书籍/资源/课程数据（31 原有 + 134 新增 + 6 PDF 直读），覆盖经典入门、自我成长、认知行为、情绪与人际、心理治疗、进化与社会、儿童发展、心理健康科普、免费资源、公开课
- `books.html` — 移除内联 libraryData → 引入 `books-data.js`；loadResources 逻辑改为 libraryData 优先 + Supabase 用户提交去重追加；书籍卡片 `<a href>` → `<div>` + `openBookDetail()` 站内弹窗；新增书籍详情弹窗（分类/访问类型/简介/推荐理由/去阅读按钮）；新增 🎲 随机推荐按钮 + 结果面板（筛选范围内随机选一本）；搜索框升级为 `.search-container` 三模式（综合/书名/作者）+ 防抖 + ×清除；新增 toast 反馈动画（推荐提交成功绿条）；移动端 CSS 完善（900/600 断点）；访问类型徽章自动识别（在线阅读/PDF下载/视频/搜书）
- `soul-tree-deploy/` — 每次修改后同步 `books.html` 和 `books-data.js`

### 达成的共识
- 鸠摩搜书/微信读书/ManyBooks/Internet Archive 等是中文心理学书籍最稳定的免费来源，公版书可用 Project Gutenberg 直接 PDF 下载
- 大部分当代心理学书籍无合法 PDF 直链——用免费阅读平台链接替代，在卡片上标注访问类型
- 书籍交互应为站内优先（弹窗详情），外链作为辅助操作（弹窗内"去阅读"按钮）
- 源文件与 `soul-tree-deploy/` 每次修改后需手动双向同步
- Playwright 测试每次修改后必须全量通过再提交
